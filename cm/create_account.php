...
// Add this function to create_account.php
function generateUniqueAccountNumber($conn) {
    do {
        $account_number = date('Ymd') . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
        $stmt = $conn->prepare("SELECT COUNT(*) FROM user_accounts WHERE account_number = ?");
...
try {
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => true, 'message' => 'Account created successfully', 'account_number' => $account_number, 'account_id' => $new_account_id]);
    } else {
        error_log("SQL execution failed: " . $stmt->error);
        echo json_encode(['success' => false, 'message' => 'Failed to create account: ' . $stmt->error]);
    }
} catch (Exception $e) {
    error_log("Exception in create_account.php: " . $e->getMessage());
